...
export default function CardTable({ color, title, data = [], headers=[], retreiveFunc, changeModalState }) {
  return (
    <>
      <div
        className={
            ...
            <tbody>
              {data.map((item, key) => {
                ...
                for (const property in item) {
                  indents.push(<td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                    {item[property]}
                  </td>)
                }
